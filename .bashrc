    ...
    for j in {0..12}; do
        case "$j" in
            ...
            5)
                ...
                ;;
            6)
                out+="${fgred}Memory: $(grep MemFree /proc/meminfo | awk {'print $2'})kB(Free)"
                ;;
	    7)
                out+="${fgred}         / $(grep MemTotal /proc/meminfo | awk {'print $2'})kB(Total)"
                ;;
            8)
                out+="${fgred}Processes: $(ps ax | wc -l | tr -d " ")"
                ;;
            9)
                out+="${fgred}IP Address: $(ip route get 8.8.8.8 2>/dev/null | awk '{print $NF; exit}')"
                ;;
            10)
                out+="Temp: CPU: $cpuTempF°F GPU: $gpuTempF°F"
                ;;
            11)
                out+="${fgwht}The RetroPie Project"
                ;;
	    12)
		out+="${fgwht}https://retropie.org.uk"
		;;
        esac
        out+="\n"
    done
